...
def parse_feed(data):
    ...
    for n in root.iter():
        ...
        desc=text_of(n,{'description','summary','content','encoded'})
        date=text_of(n,{'pubdate','published','updated','date'})
        link=text_of(n,{'link'})
        if not link:
            for ch in list(n):
...
def classify(x):
    ...
    base=x['base_category']
    if AI.search(text) and base not in {'Indian Politics','World Politics','Geopolitics'}:
        return 'AI'
    if INDIA.search(text) and POLITICS.search(text):
        return 'Indian Politics'
    if GEO.search(text) and not INDIA.search(text):
        return 'Geopolitics'
    if MACRO.search(text):
        return 'Macro Economics'
    if SCIENCE.search(text) and not AI.search(text):
...
def norm(s):
    return re.sub(
        ...
        re.sub(
            r'[^a-z0-9 ]',' ',
            re.sub(r'\b(live|breaking|latest|updates?|explained|exclusive)\b',' ',s.lower())
        )
    ).strip()
...
def age_h(iso):
    try:
        return max(
            0,
            (datetime.now(timezone.utc)-datetime.fromisoformat(iso.replace('Z','+00:00'))).total_seconds()/3600
        )
    except:
        return 999

def stable_key(title,pub):
    return hashlib.sha1(
        ((pub or '')[:10]+'|'+' '.join(words(title)[:12])).encode()
    ).hexdigest()[:20]

def fallback_summary(c):
    desc=c['primary'].get('description','')
    sent=[s.strip() for s in re.split(r'(?<=[.!?])\s+',desc) if len(s.strip())>25]
    what=sent[0] if sent else c['primary']['title']
    n=len(c['sources'])
    why=f"This is relevant to {c['category'].lower()} and is represented by {n} source{'s' if n!=1 else ''} in this briefing. Open the source coverage for context and competing interpretations."
...
def cluster(rows):
    ...
    for i,a in enumerate(rows):
        ...
        av=vec(a['title']+' '+a['description'][:500])
        members=[a]
        used.add(i)
        ...
        for j,b in enumerate(rows):
            ...
            if abs((at-bt).total_seconds())>72*3600:
                continue

            bv=vec(b['title']+' '+b['description'][:500])
            sim=cosine(av,bv)

            wa=set(words(a['title']))
            wb=set(words(b['title']))
            inter=wa&wb
            overlap=len(inter)/max(1,min(len(wa),len(wb)))

            threshold=.31 + (.05 if a['category']!=b['category'] else 0)

            if sim>=threshold or (overlap>=.48 and len(inter)>=3):
                members.append(b)
                used.add(j)

        primary=max(
            members,
            key=lambda x:(x['source_weight'],len(x['description']))
        )
        ...
        cat=next((x for x in priority if x in cats),primary['category'])
        sources=sorted(set(m['source'] for m in members))

        score=(
            max(0,48-age_h(primary['published_at']))
            + min(20,(len(sources)-1)*7)
            + primary['source_weight']*2
            + (5 if cat in {'India','Indian Politics','World','Geopolitics','Macro Economics','AI'} else 0)
            - (3 if cat=='The Ken' else 0)
        )
        ...
        what,why,watch=fallback_summary(c)

        out.append(dict(
            cluster_key=stable_key(primary['title'],primary['published_at']),
            title=primary['title'],
...
def ai_summary_for_cluster(client,c):
    ...
    for i,a in enumerate(c.get('articles',[])[:5],1):
        ...
        articles.append(
            f"[S{i}] {a.get('source','Source')}: {a.get('title','')}\n{desc}"
        )
...
async def main():
    headers={
        'User-Agent':'DailyIntelligence-GitHubPages/1.0 personal news reader'
    }
    ...
    async with httpx.AsyncClient(
        ...
        results=await asyncio.gather(
            *[fetch(client,s) for s in SOURCES]
        )

    health=[h for _,h in results]
    rows=[x for batch,_ in results for x in batch]
    ...
    for x in rows:
        ...
        try:
            if datetime.fromisoformat(
                x['published_at'].replace('Z','+00:00')
            )<cutoff:
                continue
        except:
            pass
    ...
    summary_mode=enrich_with_ai(clusters)
    top=balanced_top(clusters,8)
    ...
    payload=dict(
        ...
        clustering_mode='lexical-cosine',
    ...
    if (
        local.hour>6
        or (local.hour==6 and local.minute>=15)
    ...
    print(
        f'Generated {len(rows)} articles, '
        f'{len(clusters)} clusters, '
        f'{sum(h["ok"] for h in health)}/{len(health)} healthy sources'
    )
